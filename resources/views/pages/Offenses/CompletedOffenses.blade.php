...
@section('Title','Completed Offenses')
...
<div class="parent">
<h2 class="text-center mb-3">Completed Offenses</h2>
<div class="container police">
            <div class="table-responsive">
                <table class="main-table text-center table table-bordered">
                  @auth("police")
                  @if(\App\Models\Offense::all()->where("paidStatut", 1)->where("Traffic_id", auth("police")->user()->id)->count() > 0)
                    <tr>
                          <td>#</td>
                    ...
                    @else
                    <div class="alert alert-info">There is no Completed Offenses yet</div>
                    @endif
                    @endauth
                    @auth("web")
                    @if(\App\Models\Offense::all()->where("paidStatut", 1)->count() > 0)
                    <tr>
                          <td>#</td>
                          <td>Offense Number</td>
                          <td>offender Name</td>
                    ...
                    @endauth
                    @auth("police")
                    @foreach(\App\Models\Offense::all()->where("paidStatut", 1)->where("Traffic_id", auth("police")->user()->id) as $Offense)
                         <tr>
                           <td>{{$loop->index+1}}</td>
                    ...
                    @endauth
                    @auth("web")
                    @foreach(\App\Models\Offense::all()->where("paidStatut", 1) as $Offense)
                    <tr>
                      <td>{{$loop->index+1}}</td>
                      <td>{{$Offense->offense_number}}</td>
                      <td>{{$Offense->offender_name}}</td>
